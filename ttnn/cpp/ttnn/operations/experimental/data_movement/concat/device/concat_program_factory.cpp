...
#include "ttnn/cpp/ttnn/operations/experimental/data_movement/concat/device/concat_program_factory.hpp"
...
#include "ttnn/cpp/ttnn/operations/experimental/data_movement/concat/device/concat_device_operation.hpp"
...
namespace ttnn::operations::experimental::data_movement::detail {

operation::ProgramWithCallbacks experimental_s2s_rm_concat_multi_core(
    ...
    // Assume inputs and output have the same element type and alignment.
...
operation::ProgramWithCallbacks s2s_rm_concat_multi_core(
    ...
    for (auto core : cores) {
        ...
        vector<uint32_t> runtime_args_1 = {
            curr_num_output_rows * num_input_tensors, curr_num_output_rows * output_stick_size};
        uint32_t page_id = div_up(curr_num_output_rows, 2);
        if (not writer) {
            runtime_args_0[3] = div_up(curr_num_output_rows, 2);
            runtime_args_1 = {
                page_id * output_stick_size,
...
operation::ProgramWithCallbacks sharded_concat_multi_core(
    ...
    if (output.is_sharded()) {
        return experimental_s2s_rm_concat_multi_core(input_tensors, dim, output);
