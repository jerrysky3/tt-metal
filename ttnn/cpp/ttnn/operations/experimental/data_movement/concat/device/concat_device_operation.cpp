...
namespace ttnn::operations::experimental::data_movement {
...
std::vector<tt::tt_metal::LegacyShape> ConcatDeviceOperation::compute_output_shapes(
    const std::vector<Tensor> &input_tensors) const {
            ...
            if (input_tensors[0].is_sharded()) {
                ...
                if (input_tensors[0].get_layout() == Layout::ROW_MAJOR && normalized_dim == ref_rank - 1) {
                    for (const auto &input_tensor : input_tensors) {
                        TT_FATAL(
                            (input_tensor.get_legacy_shape()[dim] * input_tensor.element_size()) %
                ...
                return operation::run_with_autoformat(
                    ConcatDeviceOperation{normalized_dim, output_mem_config},
                    {input_tensors},
                    {input_format_params},
                    {target_layout});
            }
