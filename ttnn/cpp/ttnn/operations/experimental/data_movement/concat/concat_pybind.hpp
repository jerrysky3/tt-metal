...
Example:
    ...
    >>> output = ttnn.experimental.concat([tensor1, tensor2], dim=4)
    >>> print(output.shape)
    [1, 1, 32, 64]

    )doc";

    using OperationType = decltype(ttnn::experimental::concat);
    ttnn::bind_registered_operation(
        ...
        ttnn::pybind_overload_t{
            [](const OperationType& self,
               ...
               uint8_t queue_id) { return self(queue_id, tensors, dim, memory_config, optional_output_tensor); },
            py::arg("tensors"),
            py::arg("dim") = 0,
            py::kw_only(),
